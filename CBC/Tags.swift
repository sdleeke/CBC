...
class Tags
{
    ...
    var selected:String?
    {
        ...
        set {
            ...
            guard (media?.tagged[newValue] == nil) else {
                return
            }
            
            guard media?.all != nil else {
                //This is filtering, i.e. searching all mediaItems => s/b in background
                media?.tagged[newValue] = MediaListGroupSort(mediaItems: media?.repository.list?.filter({ (mediaItem) -> Bool in
                    return mediaItem.category == media?.category.selected
                }).withTag(tag: newValue))
                return
            }

            media?.tagged[newValue] = MediaListGroupSort(mediaItems: media?.all?.tagMediaItems?[newValue.withoutPrefixes])
        }
    }
}
