            ...
            if document?.showing(selectedMediaItem) == true {
                Thread.onMain { [weak self] in
                    ...
                    if let purpose = self?.document?.purpose {
                        switch purpose {
                        case Purpose.slides:
                            self?.networkUnavailable("Slides unavailable.")
                            break
...
//        }
    ...
    func setDocumentContentOffset(_ document:Document?)
    {
        ...
        Thread.onMain { [weak self] () -> (Void) in
...
//            break
    ...
    // This would be in an extension for the protocol but Swift 5 won't allow override in an extension
